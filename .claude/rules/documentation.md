...
> 本文件是 agent 写文档时的强制规范。
> 触发时机:写/改任何 .md 文档前;`git commit` 前(pre-commit-doc-check hook 自动提醒)。
...
**禁止内容**:架构详解(放 ADR)、数据流图(放 `docs/architecture/data-flow/`)、版本变更摘要(放 ADR 的 Alternatives 段)、环境变量完整列表(放 `.env.example`)。
...
任何涉及以下模块的改动,启动时必须先读 [`docs/architecture/rules.md`](../../docs/architecture/rules.md):
