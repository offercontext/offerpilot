...
v2 不用一份 JSON 替代两个阶段，而新增 `opportunity_fit_review_stages` 表，每个阶段一行：`id`、`application_id`、`resume_id`、可空 `parent_triage_stage_id`、`stage`（仅 `triage|deep_review`）、`proposal_schema_version=2`、`idempotency_key`、`source_snapshot_json`、`source_fingerprint_sha256`、`proposal_json`、`proposal_sha256`、`status`、`confirmed_at`、`created_at`。`(application_id, stage, idempotency_key)` 唯一；`deep_review` 必须引用同投递的 `triage` stage。这里的 `proposal_json` 是单个阶段的 v2 JSON，不是把 Triage 和 Deep 合并的结果；每个阶段独立保存快照、指纹、Proposal hash 和幂等键。旧 v1 表的双阶段列不改可空，避免为 v2 破坏旧数据删除/读取语义。

Triage 生成创建不可变 `stage=triage` 行，状态为 `ready` 但未确认；用户点击“开始 Deep Review”时先用一次性确认 token 通过 CAS 写入 `confirmed_at`，不改变 Proposal JSON 或哈希。Deep Review 仅接受已确认且来源仍可见的 Triage，Provider 调用前冻结独立 Deep snapshot，调用完成后创建新的不可变 `stage=deep_review` 行；未确认 Triage 稳定拒绝。Deep 重新生成必须使用新幂等键和新 stage 行，不能覆盖旧 Deep。每个阶段同 key 且快照相同返回原行，同 key 快照不同返回 `409 opportunity_fit_idempotency_conflict`；JD/Resume/用户断言或 Application 来源漂移时旧 stage 保留只读并标 `source_changed`，新阶段必须重新生成，Provider/网络未知保留原 key。
...
现有 Triage POST 路径接收 `schema_version=2` 后只创建 Triage stage，并返回 `stage_id` 与一次性 `triage_confirmation_token`；用户确认后，现有 Deep Review 路径必须接收 `schema_version=2`、原 `review_id`、`triage_confirmation_token` 和 Deep 专用 `idempotency_key`，服务端先 CAS 确认 Triage，再调用 Provider。列表/详情返回 v1 历史行与 v2 stage 历史的联合结果；任何旧 v1 行都不能通过 v2 Deep 路径改写。
