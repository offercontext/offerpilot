...
|---|---|---|---|---|
| 投递 | 左栏“投递”看板/列表/日历，详情可恢复；Application、JD 文本、Events、Resume/Material Kit | 手工 CRUD/状态变更；Chat 写入另有 pending confirmation；`AddApplicationForm` 只保存 `job_url`。当前 UI 的 `JDAnalyzeModal` 和 `ResumeMatchModal` 只传 `jd_text` | 正式可用；`ApplicationDetail.tsx`、`AddApplicationForm.tsx`；`tests/test_applications_api.py`、`tests/test_events_api.py`、`scripts/local-smoke.ps1` | Wiki P0/P1 基本一致。`POST /api/jd/analyze` 与 `/api/resumes/{id}/match` 仍接受 `jd_url`，但未找到 UI 传 URL 的调用点；属于可直接调用的遗留候选，不应直接扩大 P0 |
...
| Offer | 投递模块 Offer 中心、详情/Command Palette；Offer 手工字段可恢复 | 仅手工保存；现有金额、周期、签字费、权益、截止期，无 AI 自动写入 | 正式可用但比较未收敛；`OfferCenterView.tsx`、`AddOfferForm.tsx`、`OfferCompareDrawer.tsx`、`repositories/offers.py`、`/api/offers/compare`；`tests/test_offers_api.py`、`scripts/local-smoke.ps1` | Wiki Offer 中心 v0.2 一致；现有比较允许少于两个，且无口径校验（缺陷） |
...
已核实的调用链是：前端 `JDAnalyzeModal.tsx → services/ai.ts → POST /api/jd/analyze`，前端实际只传 `jd_text`；`ResumeMatchModal.tsx → services/resumes.ts → POST /api/resumes/{id}/match`，也只传 `jd_text`。服务端两路在缺少文本时才读取 `jd_url`，进入 `api.py:_fetch_text_from_url` 的 `httpx.get`；`tests/test_jd_resume_ai_api.py` 只覆盖文本和缺失输入，没有 URL 负向测试。结论是“外部请求代码和 API 分支真实存在，但当前主 UI 调用 URL 未被证明”；后续先加路由/请求审计和 URL 负向测试，再决定隐藏、移除或保留兼容分支，不把死/遗留分支直接升格为 P0 主流程缺陷。
...
| Material Proposal 的“接受/拒绝变更”按钮 | 保留 | 这是用户逐项确认 AI 改写，不是模型替用户做 Offer/Application 决策，不应删除 |

新生成的 Opportunity Fit 不再接受上述三个枚举作为面向用户的决策字段；实现阶段以兼容读取适配旧快照，新增响应使用条件/风险/问题结构。其他 Proposal 遵守同一显示原则：证据不足为安全空结果，不分数、不预测、不替用户决定。

### 3.2 Agent 写入状态机、审计和配置迁移
...
| 流程 | 输入/证据 | Provider 未知 | 契约失败 | 安全空/无证据 |
|---|---|---|---|---|
| 材料 | Application、Kit、Resume、可选 Evidence Bundle/断言 | `502`，保留 key/冻结输入 | 一次修复后 `201 safe_empty`，不写未验证 draft | `safe_empty`，不接受、不派生 Resume |
| 机会评估 | 当前 Application、JD、用户选 Resume/断言 | 结果未知，保留 attempt key | 明确不可验证 `502`，不写 Review | 固定资料不足结果；不产生决策 |
| 面试复盘 | Note 与 interview event 元数据 | `502`，保留 key | 两次失败后安全空 Proposal `201` | 空建议只能说暂无可验证建议 |
...
四类终态必须分别测试和映射：Provider/网络/超时是结果未知；JSON/字段/证据/上限失败是契约失败；经过服务端校验的固定空结构是安全空结果；输入没有任何可用证据是无证据可用，返回中文资料不足而不是 Provider 错误。各流程保留自己的输入白名单、证据路径和错误码，只抽取最小 JSON/证据/CAS 底层，不做大一统重构。

### 4.3 URL 和系统文案

`job_url/jd_url` 统一为用户记录的来源字符串：保存、展示、历史快照可保留，但不代表抓取、登录或访问招聘平台。先对 `/api/jd/analyze`、`/api/resumes/{id}/match` 增加“带 URL 不发外部请求/只接受明确文本”的负向测试，并记录现有 `httpx.get` 路径，再决定兼容 API 的隐藏或移除；不把它混入 P0 面试/投递索引改造。
...
顺序：P0-A Agent 写权限和旧决策语义适配（回归 Chat/工具/旧 Proposal 快照）；P0-B 顶层面试索引与可见性/404（回归事件、Note、历史 Proposal、Knowledge）；P0-C Offer 两条护栏（回归 API/UI 0/1/2 和旧数据）；P1-A 统一行动规则（规则函数、时区、阈值、导航）；P1-B 五类 Proposal 差异矩阵与最小公共校验（契约、证据、并发、未知重试、隔离 real-AI）；P1-C URL 兼容边界和固定中文扫描。

每阶段风险和回归范围如上，最终还需 `uv run pytest`、`uv run ruff check .`、`uv run mypy src`、前端测试/构建、`scripts/local-smoke.ps1`、隔离 local/real-AI verify 和真实浏览器走查。真实走查只能使用临时数据目录，断言无跨领域写入、无招聘平台请求并清理残留。本轮不执行实现测试。

## 6. 一致性结论与复审项

代码已领先主 Wiki 的旧版本标记，尤其材料、机会评估、面试复盘/知识/准备；这是文档滞后，不是新增能力。Knowledge Source/Evidence/Note Version 与本地架构及 ADR-0003/0004 一致。待复审确认的最小决策是：URL 分支先按遗留候选核实、Agent 控制面保留审计但业务零写入、旧决策枚举只读兼容、Offer 护栏先于口径模型、面试索引先复用可见性契约。通过后再写测试先行计划，当前不进入代码调整。
