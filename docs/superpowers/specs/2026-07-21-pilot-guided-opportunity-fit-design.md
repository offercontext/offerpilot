...
`AppShell` 持有内存中的一次性 `MaterialKitHandoff`，由 Pilot 卡片写入、由 `ApplicationDetail` 消费。令牌不落库、不进入 URL、不发送给 AI：

```ts
type MaterialKitHandoff = {
  token: string;
  ...
  └─ 用户点击“准备材料”
      └─ AppShell 写入一次性 handoff
          └─ 打开 ApplicationDetail 的 Material Kit 抽屉
              └─ ApplicationDetail 校验 applicationId/token 并消费
                  └─ MaterialKitDrawer 只读预填 resumeId/jdText
```

消费后立即清除令牌；刷新前未提交的输入只存在 React 状态，刷新后不恢复。历史评估恢复不使用旧的临时输入，而是读取详情接口返回的冻结快照。
...
输入限制在前端即时提示并禁用按钮，后端 422 仍作为最终防线。输入状态不写入 Application、Resume、Material Kit 或 Opportunity Fit 记录，直到用户确认并成功调用 Triage API。
...
只有 `recommended_path=prepare_materials` 或用户明确选择“准备材料”时，才显示材料交接按钮。该按钮只触发一次性交接，不自动生成材料、不打开投递操作。
...
3. Triage 失败不写新的评估；Deep Review 失败不覆盖已有结果。
4. 相同幂等键沿用后端幂等返回，不重复创建评估。
5. Provider 失败、未验证输出、404、422 和未知错误均不产生客户端伪造结果。
...
- 证据标签、引用路径/摘录原文保留；
- “准备材料”写入 handoff 并打开现有抽屉，未调用生成 Material Kit 或接受 Proposal；
...
`real-ai` smoke 使用临时隔离数据目录创建合成 Application、Resume、JD 和断言，调用现有 Triage/Deep Review API，允许模型返回安全的空结果；测试结束清理临时目录，不污染用户数据库。所有失败只保留安全类别，不记录原始模型输出或敏感快照。
