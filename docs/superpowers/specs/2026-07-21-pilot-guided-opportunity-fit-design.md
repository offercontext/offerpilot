...
AppShell 只提供以下消费接口，不暴露 handoff 的可写 state：

consumeMaterialKitHandoff(applicationId: number): MaterialKitHandoff | null;

该函数在一次同步操作中检查 `applicationId` 是否匹配，匹配时返回记录并立即清除，不匹配或重复消费返回 `null`。ApplicationDetail 只能消费一次匹配记录，Pilot 卡片不能通过 props 或共享引用继续修改已交接对象。
```
