...
| Local release smoke | Complete | `scripts/local-smoke.ps1` and `scripts/local-smoke.sh` build the SPA, start `oc start`, check health and SPA fallback, then run `oc smoke`. |
