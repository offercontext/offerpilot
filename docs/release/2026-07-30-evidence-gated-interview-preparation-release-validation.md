...
- `uv run pytest tests/test_mock_interview_ai.py -q`: exit 0, 26 passed.
- All Mock Interview test files (`tests/test_mock_interview*.py`): exit 0, 103 passed.
...
- `uv run oc verify --profile real-ai --static-dir web/dist`: exit 0.
  ...
  - Mock Interview bounded attempts ended with `attempt_1:feedback:mock_interview_unverifiable:excerpt_mismatch`, then `attempt_2:success`; the terminal failure was preserved and the successful retry used a new Attempt.
