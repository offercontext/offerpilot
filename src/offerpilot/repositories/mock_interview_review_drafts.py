...
class MockInterviewReviewDraftRepository:
    ...
    def confirm_review_draft(
        ...
        if not confirmation_idempotency_key:
            raise MockInterviewReviewDraftValidationError(
                "confirmation_idempotency_key is required"
            )
        ...
        with self._session_factory() as session:
            ...
            if proposal is None or proposal.proposal_status != "normal":
            ...
            attempt = session.get(MockInterviewAttempt, proposal.attempt_id)
            if attempt is None:
                raise MockInterviewReviewDraftValidationError("attempt not found")
