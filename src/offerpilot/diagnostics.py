...
def _count_valid_log_rows(handle: BinaryIO, boundary: int) -> int:
    return sum(_is_valid_log_row(raw) for raw in _iter_snapshot_lines_forward(handle, boundary))
...
def _is_valid_log_row(raw: bytes) -> bool:
    return _parse_log_object(raw) is not None
