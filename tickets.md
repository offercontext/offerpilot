...
- [x] 每条报告项至少包含 block path、issue type、decision、reason 和 evidence IDs。
...
- [x] 失败详情不把完整 Evidence 正文复制进 report 或普通日志；详情按 Evidence ID 从本地数据读取。
...
- [x] 定义固定版本 repair patch Schema，操作只包含失败 block 的 replace、delete 或 split。
...
- [x] Repair 可以为失败 block 增加、替换或删除当前 Source/Snapshot citations，即使新 Evidence ID
      不在原候选中。
- [x] Repair 不得修改已通过 block、引用其他 Source/Snapshot、增加新主题或输出完整 Brief。
