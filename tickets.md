...
- [x] 来源/导出格式适配器按确定性结构信号选择：Obsidian 由文档级 `![[...]]`/`%%...%%` 触发、Evernote 由 `<en-*>`/`.enex` 触发、web 文章由 ingest `origin_url` 触发；一个信号只启用自身适配器，品牌名称、正文关键词与文件标题不作信号，无任何信号时平台规则不执行、相关正文默认保留。
...
- [x] Repair 可以为失败 block 增加、替换或删除当前 Source/Snapshot citations，即使新 Evidence ID
      不在原候选中。
- [x] Repair 不得修改已通过 block、引用其他 Source/Snapshot、增加新主题或输出完整 Brief。非 guide 块的 replace/split 受章节边界约束：新引用必须落在原块有效 citation 所属章节集合内，越出即拒绝；原块无有效 citation 可定章节时只允许 delete。
...
- [x] reset 采用原子 quarantine：`knowledge/` 先原子移出到同文件系统 quarantine，再提交 DB 单事务；DB 提交为逻辑完成点，提交失败原子移回，quarantine 清理失败只记 pending 待启动恢复扫除而不回退。
